...
The command prints a concise note based on the canvas weekender's delivery and return details. In an actual checkout route, pass the product facts already rendered on the page to `createCheckoutReassurance`.
...
The important part is ordinary OpenAI client code. `model: "auto"` allows Infrai to route across model vendors without putting vendor branches into the checkout feature. The same `INFRAI_API_KEY` is used by the feature, so the storefront team keeps one credential for this AI call.
...
The helper retries a 429 with exponential delay and uses `Retry-After` when it is present. Other responses are surfaced to the caller, which lets the checkout route retain its own existing fallback copy.

## The checkout detail that matters

The real gotcha is allowing generated text to fill in missing store policy. The prompt takes only named item, delivery, and returns values, then tells the model to stay factual. Keep any policy calculation in the storefront and pass the final customer-facing values here.
